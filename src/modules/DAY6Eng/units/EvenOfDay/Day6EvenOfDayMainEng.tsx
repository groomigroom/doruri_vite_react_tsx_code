import { Link } from 'react-router-dom';
import { EngPage } from '../../../HomeEng/EngPage';

import '../../../triples/main/TriplesMain.css';
import '../../../DAY6/units/EvenOfDay/Day6EvenOfDayMain.css';

import MenuEng from '../../../HomeEng/MenuEng';
import FooterPartEng from '../../../FooterPartEng';

// import Day6EvenOfDay_m_logo from './img/Day6EvenOfDay_m_logo.svg';
import MainH2Deco from '../../../img/main_h2_deco.png';
// import Day6EvenOfDay_m_img from './img/Day6EvenOfDay_m_img.jpg';

// import Day6EvenOfDay_youngk_img from './img/Day6EvenOfDay_mem_youngk_img.jpg';
// import Day6EvenOfDay_wonpil_img from './img/Day6EvenOfDay_mem_wonpil_img.jpg';
// import Day6EvenOfDay_dowoon_img from './img/Day6EvenOfDay_mem_dowoon_img.jpg';

//#191919

export default function Day6EvenOfDayMainEng() {
    EngPage('en', 'ko');

    return (
        <div className='day_EvenOfDay_m_full'>
            <MenuEng />
            <section className="home_translate">
                <Link to='/Day6EvenOfDayMain'>한국어</Link>
                <Link to='/Day6EvenOfDayMainEng' className="on">English</Link>
            </section>
            <img src='' alt="DAY6 (Even of Day) logo" className='tri_m_logo' />
            <h2 className='tri_m_h2 tri_m_h2_1'>
                <img src={MainH2Deco} alt="DAY6 Even of Day deco" />&apos;DAY6 &#40;Even of Day&#41;&apos;<img src={MainH2Deco} alt="DAY6 Even of Day deco" />
            </h2>
            <p className='tri_m_intropp'>
                Even if there&apos;s a heart<br />
                scattering with &apos;DAY6 &#40;Even of Day&#41;&apos;,<br />
                let&apos;s stay strong and see!
            </p>
            <img src='' className='tri_m_img' alt='Even of Day' />
            <h2 className='tri_m_h2 tri_m_h2_2'>▪ members ▪</h2>
            <section className='tri_m_mem_tr_full day_EvenOfDay_m_mem_tr_full'>
                <section className='tri_m_mem_tr'>
                    <Link to='/DayMemWonpilEng' className='tri_m_mem_a'>
                        <article className='tri_m_mem'>
                            <img src='' alt="Wonpil" />
                            <div>
                                <h4>Wonpil</h4>
                            </div>
                        </article>
                    </Link>
                    <Link to='/DayMemDowoonEng' className='tri_m_mem_a'>
                        <article className='tri_m_mem'>
                            <img src='' alt="Dowoon" />
                            <div>
                                <h4>Dowoon</h4>
                            </div>
                        </article>
                    </Link>
                    <Link to='/DayMemYoungKEng' className='tri_m_mem_a'>
                        <article className='tri_m_mem'>
                            <img src='' alt="Young K" />
                            <div>
                                <h4>Young K</h4>
                            </div>
                        </article>
                    </Link>
                </section>
            </section>

            {/* 모바일 보기용 /}
{/ <section className="tri_m_mem_tr_full_mobile epi_m_mem_tr_full_mobile">
<section className="tri_m_mem_trtr">
<section className='tri_m_mem_tr'>
                            <Link to='/EpiMemTablo' className='tri_m_mem_a'>
                                <img src="" alt="타블로 이미지" />
                                <h4>타블로</h4>
                            </Link>
                            <Link to='/EpiMemTwokutz' className='tri_m_mem_a'>
                                <img src="" alt="투컷 이미지" />
                                <h4>투컷</h4>
                            </Link>
                        </section>
                </section>
                <section className="tri_m_mem_trtr">
                    <section className='tri_m_mem_tr1'>
                        <Link to='/EpiMemMithra' className='tri_m_mem_a'>
                            <img src="" alt="미쓰라 이미지" />
                            <h4>미쓰라</h4>
                        </Link>
                    </section>
                </section>

            </section> */}

            <FooterPartEng FooterPartEngText='ⓒ JYP ENTERTAINMENT' />
        </div >
    );
};
