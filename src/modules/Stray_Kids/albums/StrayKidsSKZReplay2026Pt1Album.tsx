import { Link } from 'react-router-dom';

import '../../triples/albums/TriplesBabyFlowerCityRemixesAlbum.css';
import './StrayKidsSKZReplay2026Pt1Album.css';

import Menu from '../../Menu';
import FooterPart from '../../FooterPart';

import StrayKidsSKZReplay2026Pt1Album_img from './img/StrayKidsSKZReplay2026Pt1Album_img.jpg';

import MainH2deco from '../../img/main_h2_deco.png';

export default function StrayKidsSKZReplay2026Pt1Album() {
    return (
        <div className='str_StrayKidsSKZReplay2026Pt1Album_full'>
            <Menu />
            <section className="home_translate">
                <Link to='/StrayKidsSKZReplay2026Pt1Album' className="on">한국어</Link>
                <Link to='/StrayKidsSKZReplay2026Pt1AlbumEng'>English</Link>
            </section>
            <h2 className='tri_BabyFlowerCityRemixesAlbum_h2 tri_BabyFlowerCityRemixesAlbum_h2_1'>
                <img src={MainH2deco} alt="Stray Kids 앨범 꾸미기" />SKZ-REPLAY 2026 Pt.1<img src={MainH2deco} alt="Stray Kids 앨범 꾸미기" />
            </h2>
            <p className='tri_BabyFlowerCityRemixesAlbum_intropp'>
                다양한 곡들을<br />
                모아서<br />
                앨범을 준비했어요!
            </p>
            <img src={StrayKidsSKZReplay2026Pt1Album_img} alt="StrayKidsSKZReplay2026Pt1Album" className='tri_BabyFlowerCityRemixesAlbum_img' />
            <table className='tri_BabyFlowerCityRemixesAlbum_tb1'>
                <tbody>
                    <tr className='tri_BabyFlowerCityRemixesAlbum_intropa_tb1_tr1'>
                        <td>
                            <p>
                                발매일
                            </p>
                        </td>
                        <td>
                            <p>
                                2026.08.01.
                            </p>
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <p>
                                장르
                            </p>
                        </td>
                        <td>
                            <p>
                                발라드, 랩/힙합, 댄스, R&B/Soul, 재즈
                            </p>
                        </td>
                    </tr>
                    <tr className='tri_BabyFlowerCityRemixesAlbum_intropa_tb1_tr3'>
                        <td>
                            <p>
                                기획사
                            </p>
                        </td>
                        <td>
                            <p>
                                JYP엔터테인먼트
                            </p>
                        </td>
                    </tr>
                </tbody>
            </table>
            <h2 className='tri_BabyFlowerCityRemixesAlbum_h2 tri_BabyFlowerCityRemixesAlbum_h2_2'>▪ 곡 목록 ▪</h2>
            <table className='str_StrayKidsSKZReplay2026Pt1Album_tb2'>
                <tbody>
                    <tr className='str_StrayKidsSKZReplay2026Pt1Album_intropa_tb2_tr1'>
                        <td>
                            <p>
                                번호
                            </p>
                        </td>
                        <td>
                            <p>
                                제목
                            </p>
                        </td>
                        <td>
                            <p>
                                가수
                            </p>
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <p>
                                1
                            </p>
                        </td>
                        <td>
                            <p>
                                Battle Ground &#40;Korean Version&#41;
                            </p>
                        </td>
                        <td>
                            <p>
                                Stray Kids
                            </p>
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <p>
                                2
                            </p>
                        </td>
                        <td>
                            <p>
                                Roman Empire
                            </p>
                        </td>
                        <td>
                            <p>
                                Stray Kids, 방찬
                            </p>
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <p>
                                3
                            </p>
                        </td>
                        <td>
                            <p>
                                LOVER
                            </p>
                        </td>
                        <td>
                            <p>
                                Stray Kids, 현진
                            </p>
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <p>
                                4
                            </p>
                        </td>
                        <td>
                            <p>
                                유성우 &#40;Raining stars&#41;
                            </p>
                        </td>
                        <td>
                            <p>
                                Stray Kids, 한
                            </p>
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <p>
                                5
                            </p>
                        </td>
                        <td>
                            <p>
                                행복 뭐 별거 없네요
                            </p>
                        </td>
                        <td>
                            <p>
                                Stray Kids, 아이엔
                            </p>
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <p>
                                6
                            </p>
                        </td>
                        <td>
                            <p>
                                Goodbye
                            </p>
                        </td>
                        <td>
                            <p>
                                Stray Kids, 승민
                            </p>
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <p>
                                7
                            </p>
                        </td>
                        <td>
                            <p>
                                제자리걸음 &#40;Still here&#41;
                            </p>
                        </td>
                        <td>
                            <p>
                                Stray Kids, 승민
                            </p>
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <p>
                                8
                            </p>
                        </td>
                        <td>
                            <p>
                                급류
                            </p>
                        </td>
                        <td>
                            <p>
                                Stray Kids, 승민
                            </p>
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <p>
                                9
                            </p>
                        </td>
                        <td>
                            <p>
                                MIC&BRUSH
                            </p>
                        </td>
                        <td>
                            <p>
                                Stray Kids, 현진
                            </p>
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <p>
                                10
                            </p>
                        </td>
                        <td>
                            <p>
                                PERFUME
                            </p>
                        </td>
                        <td>
                            <p>
                                Stray Kids, 승민
                            </p>
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <p>
                                11
                            </p>
                        </td>
                        <td>
                            <p>
                                BABY
                            </p>
                        </td>
                        <td>
                            <p>
                                Stray Kids, 방찬
                            </p>
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <p>
                                12
                            </p>
                        </td>
                        <td>
                            <p>
                                Don&apos;t Say
                            </p>
                        </td>
                        <td>
                            <p>
                                Stray Kids, 한
                            </p>
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <p>
                                13
                            </p>
                        </td>
                        <td>
                            <p>
                                명
                            </p>
                        </td>
                        <td>
                            <p>
                                Stray Kids, 창빈
                            </p>
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <p>
                                14
                            </p>
                        </td>
                        <td>
                            <p>
                                REV IT UP
                            </p>
                        </td>
                        <td>
                            <p>
                                Stray Kids, 필릭스
                            </p>
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <p>
                                15
                            </p>
                        </td>
                        <td>
                            <p>
                                나지막이 &#40;Live&#41;
                            </p>
                        </td>
                        <td>
                            <p>
                                Stray Kids, 리노
                            </p>
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <p>
                                16
                            </p>
                        </td>
                        <td>
                            <p>
                                안아줄게요 &#40;Live&#41;
                            </p>
                        </td>
                        <td>
                            <p>
                                Stray Kids, 아이엔
                            </p>
                        </td>
                    </tr>

                    <tr className='str_StrayKidsSKZReplay2026Pt1Album_intropa_tb2_tr9'>
                        <td>
                            <p>
                                17
                            </p>
                        </td>
                        <td>
                            <p>
                                별, 빛 &#40;STAY&#41;
                            </p>
                        </td>
                        <td>
                            <p>
                                Stray Kids
                            </p>
                        </td>
                    </tr>
                </tbody>
            </table>
            <FooterPart FooterPartText='ⓒ JYP엔터테인먼트' />
        </div>
    );
}
