...
React TSX(또는 JSX)에서 .filter()는 배열에서 원하는 특정 조건에 맞는 데이터만 걸러내어 새로운 배열을 만드는 자바스크립트 내장 함수(Method)입니다. 원본 배열은 절대 변경하지 않습니다. [1, 2] 
UI 개발을 할 때 특정 항목을 화면에서 지우거나(삭제 기능), 검색어에 맞는 항목만 보여주거나(검색 기능), 특정 카테고리만 분류할 때 필수적으로 사용됩니다. [2, 3] 
## 🛠️ 핵심 동작 원리
.filter()는 배열의 모든 요소를 하나씩 돌면서 콜백 함수를 실행합니다. [1] 

* 
* 콜백 함수가 true를 반환하는 요소만 남깁니다.
* 콜백 함수가 false를 반환하는 요소는 버려집니다. [2, 4, 5] 
* 
